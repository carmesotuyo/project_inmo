...
export class Filters {
  ...
  public notify = async (input: SensorData): Promise<SensorData> => {
    ...
    try {
      if (input.needsAlert) {
        const priority = (await this.checkPriority(input)) as NotificationPriority;
        let typeValue = NotificationType.SensorSignal;
        if (input.sensorId.includes('APP')) typeValue = NotificationType.AppIncident;
        const { propertyKey, propertyValue } = await this.extractKeyValue(input);
        const [propertyId, sensorId] = await this.getPropertyAndSensorIds(input);
        const notifRequest: NotificationRequest = { type: typeValue, propertyId: propertyId, priority: priority, message: propertyKey + propertyValue }; //check format of notification
        ...
        this.queueService.addJobToQueue(dataToReport as unknown as JSON);
      }
      return input;
    } catch (error: any) {
  ...
  private checkPriority = async (input: SensorData): Promise<string> => {
    const [propertyId, sensorCatalogId] = await this.getPropertyAndSensorIds(input);
    ...
    if (sensorCatalogId != 'APP') {
      ...
      return serviceType?.get('priority') as string;
    } else return 'High'; //asumimos prioridad alta para los incidentes reportados desde la app
  };
}
